...
WallbangEverything(player)
{
    ...
    if(isDefined(player.WallbangEverything))
    {
        ...
        while(isDefined(player.WallbangEverything))
        {
            ...
            /*
            IF ONLY WANT TO RUN THIS SCRIPT FOR A SPECIFIC WEAPON/WEAPON CLASS DO IT HERE
            
            EXAMPLE:

            if(weapon != "cheytac_mp")
                continue;
            */
            ...
            while(1)
            {
                ...
                if(isDefined(curEnt) && IsPlayer(curEnt) && IsAlive(curEnt))
                {
                    if(isDefined(oldPos)) //If a player was found using the initial trace, then the player is visible and MagicBullet isn't needed
                    {
                            ...
                            trace["entity"] thread [[ level.callbackPlayerDamage ]](player, player, damage, 0, "MOD_RIFLE_BULLET", "copter", player.origin, anglesF, "none", 0);
                        #else
...
doBots()
{
    gametype = level.currentGametype;
