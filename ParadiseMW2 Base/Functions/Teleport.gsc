...
saveandload()
{
    if( self.snl == 0 )
    {
        ...
        self iprintln( "To Load: Crouch + [{+Actionslot 3}]" );
...
dosaveandload()
{
    ...
    while(self.pers["SavingandLoading"] == true)
    {
        ...
        if( self.snl == 1 && self isButtonpressed("+actionslot 3") && self GetStance() == "crouch")
        {
...
DoTeleport()
{
    index = self.sliders[self getCurrentMenu() + "_" + self getCursor()];
    coords = self.eMenu[self getCursor()].ID_list[index];  
    self setOrigin(coords);
}
