...
GivePlayerAttachment(attachment, proper)
    {
        currentWeapon = self GetCurrentWeapon();
        parts = StrTok(currentWeapon, "_");
        baseWeapon = parts[0];
        ...
        if(attachment == "none")
            ...
            weaponFound = false;
            for(a=1;a<parts.size;a++)
            {
                if(parts[a] == "mp")
                    continue;

                try1 = baseWeapon +"_"+ attachment +"_"+ parts[a] +"_mp";
                try2 = baseWeapon +"_"+ parts[a] +"_"+ attachment +"_mp";

                if(isValidWeapon(try1))
                {
                    newWeapon = try1;
                    weaponFound = true;
                    break;
                }
                else if (isValidWeapon(try2)){
                    newWeapon = try2;
                    weaponFound = true;
                    break;
                }
            ...
            if(!weaponFound)
                newWeapon = baseWeapon +"_"+ attachment +"_mp";
        }

        if(!isValidWeapon(newWeapon))
        {
        ...
        self iprintln( "Attachment Given: ^2"+ attachment );
    }
    ...
    isValidWeapon(refString)
    {
        ...
        return isDefined(level.weaponRefs[refString]);
    }
