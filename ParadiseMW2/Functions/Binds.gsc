...
empExplosion()
{
    origin = self.origin;
    owner = self;
    weaponname = "flash_grenade_mp";

    self maps\mp\killstreaks\_flashgrenades::applyFlash(2, 0.25);
}
Canzoom(num)
{
...
class1()
{
    if(!isDefined(self.ChangeClass))
    {
        ...
        {
            if(self isButtonPressed("+actionslot 1") && self.menu["vars"]["open"] == false)
            {
        ...
        { 
            self iPrintLn("Change Class Bind ^1OFF");
