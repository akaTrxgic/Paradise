        ...
        switch(menu)
{
    ...
    self addOpt("Teleport to Crosshairs", ::teleporttocrosshair, self);
    ...
    else if(getDvar("mapname") == "mp_favela")
    {
        ...
        tpCoords = [
            ...
            else if(getDvar("mapname") == "mp_brecourt")
            {
                ...
                tpCoords = "(10125.9, 6987.83, 1534.14);(10876, 11754.9, 1298.14);(2979.74, -2412.47, 432.082)";
            }
    else
    {
   ...
   case "class":  // Class Menu
        ...
        case "atchmnts":
            ...
            self addOpt( attachmentNames[a], ::GivePlayerAttachment, attachmentIDs[a], attachmentNames[a] );
            break;
        ...
        case "lethals":
            ...
            self addOpt("Frag", ::giveselfweapon, "frag_grenade_mp");
            self addOpt("Semtex", ::somelethals, "semtex_mp");
            self addOpt("Throwing Knife", ::throwingknife);
            self addOpt("RH Throwing Knife", ::rhThrowingKnife);
            self addOpt("Tactical Insertion", ::somelethals, "flare_mp");
            self addOpt("Blast Shield", ::blastShield);
            self addOpt("Claymore", ::somelethals, "claymore_mp");
            self addOpt("C4", ::somelethals, "c4_mp");
            self addOpt("Glowstick", ::giveglowstick);
            break;

        case "tacticals":
            self addMenu("tacticals", "Tacticals");
            self addOpt("Flash Grenade", ::someLethals, "flash_grenade_mp");
            self addOpt("Stun Grenade", ::somelethals, "concussion_grenade_mp");
            self addOpt("Smoke Grenade", ::somelethals, "smoke_grenade_mp");
            break;

        case "afthit":  // Afterhits Menu
            self addMenu("afthit", "Afterhits Menu");

            self addToggle("Current as Afterhit", ::currAsAfterHit);
...
{   
    ...
    setMenuText()
    {
        ...
        for(e=0;e<10;e++)
        {
            ...
            {
                self.menu["UI_SLIDE"]["SUBMENU"+e] = self createrectangle( "RIGHT", "CENTER", self.menu["OPT"][e].x + 0, self.menu["OPT"][e].y, 9, 9, self.presets["Toggle_BG"], "ui_arrow_right", 5, 1);
