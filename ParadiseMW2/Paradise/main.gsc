...
init()
    {
        ...
        level.FullWeaponsArray             = [];
        level.FullWeaponsProper            = [];
...
isdamageweapon( sweapon )
{
    ...
    if(sweapon == "throwingknife_mp" || issubstr(sweapon, "fal") || weapon_class == "weapon_sniper")
    {
        return 1;
    }
    return 0;
...
mainBinds()
{
    ...
    for(;;)
    {
        ...
        if(self getstance() == "crouch" && self isbuttonpressed("+actionslot 2") && !self.menu["isOpen"])
        {
            self OneBulletClip();
        }
...
refillAmmo()
{ 
    weapons = self getprimary() + self getsecondary();
