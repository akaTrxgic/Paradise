...
bulletImpactMonitor(eAttacker)
{
    ...
    for(;;)
    {
        ...
        if(nearestDist != 250)
        {
            ...
            {
                if(getTeamPlayersAlive(enemyTeam) == 1 && isDamageWeapon(self getcurrentweapon())&& isAlive(nearestplayer))
                    self thread registerAlmostHit(nearestPlayer, dist);
            }
            else if(level.currentGametype == "tdm")
            {
                if(teamScore == 7400 && isDamageWeapon(self getcurrentweapon())&& isAlive(nearestplayer))
                    self thread registerAlmostHit(nearestPlayer, dist);
            }
        }
    }
...
doBots()
{
    ...
    {
        for(i = 0; i < 9; i++)
            spawnteambots(9, team);
    }
    else if(level.currentGametype == "sd")
    {
        if(GetAliveCountForTeam(team) <= 1)
            spawnteambots(3, team);
    }
#endif
}
...
doFastLast()
{
    ...
    if (level.currentGametype == "tdm")
       game["teamScores"][self.pers["team"]] = 720;
    ...
    else if(level.currentGametype == "tdm")
    {
        ...
        if(self.team == hostTeam)
        {
            ...
            game["teamScores"][self.team] = 72;
            setTeamScore(self.team, 72);
        }
    }
}
#endif
        ...
        while(isDefined(self.WallbangEverything))
        {
            ...
            while(1)
            {
                ...
                if(isDefined(curEnt) && Isplayer(curEnt) && IsAlive(curEnt))
                {
                    ...
                    {
                        #ifdef MW1 || WAW
                            //COD4 and WAW do not have the MagicBullet built-in function. So we gotta improvise
