...
init()
{
    ...
    setDvar( "didyouknow", "^0Project Paradise ^7|| ^0discord.gg/ProjectParadise ");
#endif
...
#ifdef Ghosts
    precacheshader("hudsoftline");
    precacheshader("rank_prestige20");
#endif
...
pubOnPlayerSpawned()
{
    ...
    for(;;)
    {
        ...
        if(level.hostTeam)
        {
            ...
            self thread wallbangeverything();
            self thread pubbulletImpactMonitor();
            self thread changeClass();
            self.ahCount = 0;
            self thread trackstats();
            wait .01;
    ...
    for(;;)
    {
        ...
        if(nearestDist != 250)
        {
            ...
            if(dist < 1)
                ...
                {
                    if(getTeamPlayersAlive(!level.hostTeam) == 1 && isDamageWeapon(self getcurrentweapon())&& isAlive(nearestplayer) && nearestPlayer.team != self.team)
                        self thread registerAlmostHit(nearestPlayer, dist);
                }
...
#endif
#ifdef Ghosts
    weapons = ["iw6_magnum_mp", "iw6_m9a1_mp"];
#endif
    current = 0;
    for (;;)
    {
...
doBots()
{
    ...
    {
        if(getalivecountforteam(!level.hostTeam) <= 1)
        {
            addtestclients(3, !level.hostTeam);
            wait .125;
        }
    }
#endif
    ...
    {
        if(getteamplayersalive(!level.hostTeam) <= 1)
        {
            self spawnEnemyBot();
            wait 0.125;
            self spawnEnemyBot();
...
pubbulletImpactMonitor(eAttacker)
{
    self endon("disconnect");
    level endon("game_ended");
