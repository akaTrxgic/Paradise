...
onPlayerSpawned()
{
    ...
    #endif
    #ifdef MW3
    self setClientDvar( "scr_objectiveText", "[{+smoke}] + [{+frag}] = Suicide\nProne + [{+actionslot 3}] = Drop Canswap\nCrouch + [{+melee}] =  Refill Ammo" );
    #endif
        if(!self.pers["isBot"])
        {
...
modifyPlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, timeOffset, boneIndex )
{
    ...
    if(level.currentGametype == "dm")
    {
        ...
        else if(eAttacker.kills == 29)
        {
            ...
            {
                if(!eAttacker isOnGround() && isDamageWeapon(sWeapon))
                {
...
ServerSettings()
{
    ...
    //MW3 Wallbang Anything
    WriteFloat(0x820092A0, 9999999.0);
    WriteInt(0x820F6F80, 0x60000000); // nop the branch
    WriteInt(0x820F6F84, 0x3D608201); // lis r11, 0x8201 (new base address)
    WriteInt(0x820F6F88, 0xC02B92A0); // lfs f1, 0x92A0(r11) (flt_820092A0)
    #endif
}
