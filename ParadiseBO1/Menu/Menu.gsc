...
case "sWpns":
    ...
    break;
        ...
        case "afthit":  // Afterhits Menu
            ...
            self addOpt("Current as Afterhit", ::currasafthit);
...
{   
    ...
    getCurrEquip()
    {
        ...
        return self.custom_class[class_num]["equipment_slot"];
    }

    menuOpen()
    {
        if(!isDefined(storedEquip))
            storedEquip = self getCurrEquip();
    ...
    menuClose(storedEquip)
    {
        equipOnClose = self getcurrequip();

        if(equipOnClose != storedEquip)
            self giveweapon(equipOnClose);
        else if(equipOnClose == storedEquip)
            self giveweapon(storedEquip);

        class = self.class;
        class_num = int( class[class.size-1] )-1;
        self.custom_class[class_num]["equipment_slot"] = equipOnClose;
